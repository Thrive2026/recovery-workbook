...
- **Weekly Check-In** — a lightweight, ongoing tracker (separate from the chapter workbooks) for things that matter both for the client's recovery and for showing peer support drives down avoidable cost: appointment follow-through, medication adherence, using a coping skill instead of the ER/911, using the Warmline instead of a higher level of care, housing stability, and social connection. Each week is its own dated entry with a notes field. A rolling "last 4 weeks" summary at the top shows a percentage for each item.
- **Visit Prep** — helps a client get ready for a Primary Care, Psychiatry, Therapist, or other appointment. Each visit has a **Care Snapshot** auto-pulled from the last 2 weeks of Weekly Check-In data, starter questions by visit type (tap to add) plus room for custom questions, a checkbox for "asked at the visit," and an "After the Visit" section (what they said, any changes made, follow-up date) so the next check-in and next visit build on what actually happened. Printable as a one-pager to bring into the room.
- **Coping Skills Toolbox** — a running, client-built list of what actually helps, each rated "helped / not this time" every time it's used — so it doubles as data on what's working. A **Box Breathing exercise** is pinned permanently at the top, with an animated pacing circle for in-session grounding.
- **Anxiety Ladder** — an exposure hierarchy tool. The client and peer supporter build a list of anxiety-provoking situations from easiest to hardest, then log each attempt's anxiety before/during/after (0-10). Watching the "during" number drop over repeated attempts is real, visible progress.
- **Then & Now** (inside each SUD/MH/Comorbid workbook, via the sidebar) — a read-only side-by-side of the client's own early-chapter and late-chapter answers to related questions (e.g. "why you wanted this to change" next to "how life is different now"). No new storage — it just reads two already-saved answers.
- **Letter to My Future Self** (also in the sidebar) — the client writes a letter early on, seals it, and opens it whenever feels right. Stored in the same track's data, no separate table.
...
If you're already past that point and just adding **Weekly Check-In** and **Visit Prep** to a live database, run this instead — it's the only schema change either tool needs, since both reuse the existing `workbook_progress` table (same JSONB `data` column, just two new values for `track`):
```sql
...
If you're already past *that* and just adding **Coping Skills Toolbox** and **Anxiety Ladder**, run this instead:
```sql
alter table workbook_progress drop constraint if exists workbook_progress_track_check;
alter table workbook_progress add constraint workbook_progress_track_check
  check (track in ('sud','mh','comorbid','resources','careteam','checkin','visitprep','toolbox','exposure'));
```
Nothing else — no new tables, no new columns, no RLS changes, since the existing `workbook_progress_scoped` policy already covers any `track` value for a client the provider has access to. **Then & Now** and **Letter to My Future Self** need no migration at all — they live inside the existing `sud`/`mh`/`comorbid` rows' `data` column.
